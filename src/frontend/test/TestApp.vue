...
import IterateSettings from '../components/settings/IterateSettings.vue'
import { getTheme } from '../theme'
import ComponentsTest from './components/ComponentsTest.vue'
import GlobalSpeechSettingsTest from './components/GlobalSpeechSettingsTest.vue'
import MainLayoutTest from './components/MainLayoutTest.vue'
import McpPopupTest from './components/McpPopupTest.vue'
import OnboardingWelcomeTest from './components/OnboardingWelcomeTest.vue'
import SpeechOverlayTest from './components/SpeechOverlayTest.vue'
import ThemesTest from './components/ThemesTest.vue'
import TrialExpiredOverlayTest from './components/TrialExpiredOverlayTest.vue'

const tabs = [
  { key: 'onboarding-welcome', label: '欢迎页', component: OnboardingWelcomeTest },
  { key: 'trial-expired', label: '激活页', component: TrialExpiredOverlayTest },
  { key: 'main-ui', label: '主界面', component: MainLayoutTest },
  { key: 'mcp-popup', label: 'MCP 弹窗', component: McpPopupTest },
  { key: 'iterate-settings', label: 'iterate 设置', component: IterateSettings },
  { key: 'global-speech-settings', label: '全局语音设置', component: GlobalSpeechSettingsTest },
  { key: 'speech-overlay', label: '语音浮层', component: SpeechOverlayTest },
  { key: 'components', label: '组件库', component: ComponentsTest },
  { key: 'themes', label: '主题测试', component: ThemesTest },
]

const initialTab = new URLSearchParams(window.location.search).get('tab')
const activeTab = ref(tabs.some(tab => tab.key === initialTab) ? initialTab! : 'mcp-popup')
...
// 跟随根节点主题类，驱动 Naive UI 主题，让黑/白切换同时作用于原生组件
function readRootTheme() {
  return document.documentElement.classList.contains('dark') ? 'dark' : 'light'
}
const rootTheme = ref(readRootTheme())
const naiveTheme = computed(() => getTheme(rootTheme.value))
let themeObserver: MutationObserver | null = null
onMounted(() => {
  rootTheme.value = readRootTheme()
        ...
        <div v-if="useSimulatedWindow" class="simulated-window-container">
          ...
          <div class="simulated-window">
            ...
            <div class="window-content">
              ...
              <!-- 其他内容 -->
              <div v-else class="app-content component-content">
