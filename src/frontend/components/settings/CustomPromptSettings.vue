...
const sortableList = shallowRef<CustomPrompt[]>([])
const { start, stop: _stop } = useSortable(sortableContainer, sortableList, {
  animation: 200,
...
async function savePromptOrder() {
  try {
    const promptIds = sortableList.value.map(p => p.id)
...
async function loadConfig() {
  try {
    ...
    // 同步到拖拽列表
    sortableList.value = [...config.value.prompts]
    nextTick(() => {
...
onMounted(async () => {
  ...
  await listen('custom-prompt-config-changed', () => {
    ...
    <div v-if="config.enabled" data-guide="custom-prompt-settings">
      ...
      <div v-else class="space-y-3">
        <div ref="sortableContainer" class="space-y-3">
          <div
            v-for="prompt in sortableList"
            :key="prompt.id"
            class="bg-black-50 rounded-lg p-4 border border-black-200 shadow-sm hover:border-black-300 transition-colors sortable-item"
          >
            <div class="flex justify-between items-start mb-2">
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <n-tag v-if="prompt.is_active === false" size="small" type="warning">
                    已禁用
                  </n-tag>
                </div>
              ...
              <div class="flex gap-1 ml-4 items-center">
                <div class="drag-handle cursor-move p-1 rounded hover:bg-black-200 transition-colors" title="拖拽排序">
                  <div class="i-carbon-drag-vertical w-4 h-4 opacity-40" />
                </div>
      ...
      <n-form :model="newPrompt" label-placement="top">
        ...
        <template v-if="newPrompt.type === 'conditional'">
          ...
          <n-form-item label="是否启用">
            <n-switch v-model:value="newPrompt.is_active">
              <template #checked>
                启用
              </template>
              <template #unchecked>
    ...
    <n-modal v-model:show="showEditDialog" preset="card" title="编辑快捷模板" style="width: 600px">
      <n-form v-if="editingPrompt" :model="editingPrompt" label-placement="top">
        ...
        <template v-if="editingPrompt.type === 'conditional'">
          ...
          <n-form-item label="是否启用">
            <n-switch v-model:value="editingPrompt.is_active">
              <template #checked>
                启用
              </template>
              <template #unchecked>
