...
function createSettings() {
    ...
    try {
      ...
      try {
        ...
        if (replyConfig) {
          ...
          loopPrompt.value = config.loop_prompt ?? DEFAULT_LOOP_PROMPT
        }
      }
      catch {
  ...
  // 更新继续回复设置
  async function updateReplyConfig(config: { enable_continue_reply?: boolean, continue_prompt?: string, loop_prompt?: string }) {
    try {
      const currentReplyConfig = await invoke('get_reply_config') as any
      const nextReplyConfig = {
        ...currentReplyConfig,
        ...config,
      }
      await invoke('set_reply_config', { replyConfig: nextReplyConfig })
      ...
      if (config.loop_prompt !== undefined) {
        loopPrompt.value = config.loop_prompt
      }
