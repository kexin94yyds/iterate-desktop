...
export function useAppManager() {
  ...
  const appConfig = computed(() => {
    const config = {
      ...
      reply: {
        ...
        loopPrompt: settings.loopPrompt.value,
      },
    }
  ...
  const actions = {
    ...
    mcp: {
      ...
      handleUpdateConditionalState: mcpHandler.handleUpdateConditionalState,
      handleUpdateConditionalActive: mcpHandler.handleUpdateConditionalActive,
      handleUpdateCustomPromptOrder: mcpHandler.handleUpdateCustomPromptOrder,
      toggleMute: mcpHandler.toggleMute,
    },
    // 音频操作
    audio: {
